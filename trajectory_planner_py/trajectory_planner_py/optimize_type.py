...
# This code was taken from https://github.com/icsl-Jeon/traj_gen
...
class PolyTrajGen(TrajGen):
    ...
    def scaleMatBigInv(self,):
        ...
        for m in range(self.M):
            matSet_ = self.scaleMat(1/(self.Ts[m+1]-self.Ts[m])) # create diagonal matrix of poly coefficients times for each segment time
            if mat_ is None:
                mat_ = matSet_.copy()
            else:
                mat_ = block_diag(mat_, matSet_) # combine all diagonal matrices in big square matrix  [mat1 0; 0 mat2 ]
        return mat_
    ...
    def IntDerSquard(self, d):
        ...
        this basically finds coefficients of Q in min_x (x.T * Q *x) quadratic program cost function. For minimum snap >> x represents snap equation
